...
addCommandHandler("updatelogin",function(plr)
    ...
    fetchRemote("https://raw.githubusercontent.com/MeeShuffle/Darmowe-zasoby/main/login_system/meta.xml",function(data,error,path)
        if error==0 then
...
function download()
    ...
    fetchRemote("https://raw.githubusercontent.com/MeeShuffle/Darmowe-zasoby/main/login_system/"..files[fileCount],function(data,error,sciezka)
        if error==0 then
...
function finish()
    ...
    fileClose(meta)
    fileDelete("download.xml")
    fetchRemote("https://raw.githubusercontent.com/MeeShuffle/Darmowe-zasoby/main/login_system/version.txt",function(data,error,sciezka)
        if error==0 then
            if fileExists(sciezka) then
                fileDelete(sciezka)
            end
